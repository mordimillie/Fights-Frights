...
public class SpawnPPProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, Entity sourceentity) {
		...
		if (world.getLevelData().getGameRules().getBoolean(FightsfrightsModGameRules.DO_BOSS_RITUALS) == true) {
			if ((entity instanceof Piglin || entity instanceof PiglinBrute || entity instanceof ZombifiedPiglin) && !((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == FightsfrightsModItems.HEX.get())
				...
				FightsfrightsMod.queueServerWork(199, () -> {
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = FightsfrightsModEntities.PIGLIN_PROSECUTOR.get().spawn(_level, BlockPos.containing(entity.getX(), entity.getY(), entity.getZ()), MobSpawnType.MOB_SUMMONED);
