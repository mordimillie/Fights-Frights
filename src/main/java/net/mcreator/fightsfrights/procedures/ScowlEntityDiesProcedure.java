				...
				if ((entity instanceof ScowlEntity _datEntL8 && _datEntL8.getEntityData().get(ScowlEntity.DATA_Popped)) == false) {
					...
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("entity.generic.explode")), SoundSource.HOSTILE, (float) 0.5, 3);
						} else {
							_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("entity.generic.explode")), SoundSource.HOSTILE, (float) 0.5, 3, false);
						}
					}
					if (world instanceof ServerLevel _level)
