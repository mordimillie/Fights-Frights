	...
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (world instanceof Level _level) {
			if (!_level.isClientSide()) {
				_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("fightsfrights:mimentis_transform")), SoundSource.NEUTRAL, (float) 0.5, -1);
			} else {
				_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("fightsfrights:mimentis_transform")), SoundSource.NEUTRAL, (float) 0.5, -1, false);
			}
		}
		if (((Entity) world.getEntitiesOfClass(MimentisEntity.class, AABB.ofSize(new Vec3(x, y, z), 1, 1, 1), e -> true).stream().sorted(new Object() {
			Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
