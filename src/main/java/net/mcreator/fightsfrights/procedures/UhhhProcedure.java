	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, Entity sourceentity) {
		...
		if (world.getLevelData().getGameRules().getBoolean(FightsfrightsModGameRules.DO_BOSS_RITUALS) == true) {
			if (((entity instanceof Zombie) && !(entity instanceof ZombifiedPiglin) || entity instanceof AbstractSkeleton)
					&& !((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == FightsfrightsModItems.HEX.get())
				...
				FightsfrightsMod.queueServerWork(199, () -> {
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = FightsfrightsModEntities.GHASTLY_GENERAL.get().spawn(_level, BlockPos.containing(entity.getX(), entity.getY(), entity.getZ()), MobSpawnType.MOB_SUMMONED);
