...
public class MimicTerrantulaHurtProcedure {
				...
				if (FoundSpot == true) {
					...
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("fightsfrights:terrantula_emerge")), SoundSource.HOSTILE, (float) 0.2, -2);
						} else {
							_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("fightsfrights:terrantula_emerge")), SoundSource.HOSTILE, (float) 0.2, -2, false);
						}
					}
					if (entity instanceof MimicTerrantulaEntity) {
						((MimicTerrantulaEntity) entity).setAnimation("Dig");
					}
					FightsfrightsMod.queueServerWork(15, () -> {
						...
						FightsfrightsMod.queueServerWork(14, () -> {
							if (entity instanceof MimicTerrantulaEntity _datEntSetL)
								_datEntSetL.getEntityData().set(MimicTerrantulaEntity.DATA_HasRevealed, false);
